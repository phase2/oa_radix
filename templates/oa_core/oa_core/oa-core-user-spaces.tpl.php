...
<div class="oa-spaces tabbable clearfix <?php print $main_class; ?>">
  <?php if (count($space_groups) > 1): ?>
    <ul class="nav nav-tabs">
      <?php foreach ($space_groups as $category => $spaces): ?>
        <li class='<?php if ($category == $active): print 'active'; endif; ?>'>
          <a href="#<?php print 'tab-' . $category ?>" data-toggle="tab">
            <?php print $category; ?>
          </a></li>
      <?php endforeach; ?>
    </ul>
  ...
  <div class="tab-content">
    <?php foreach ($space_groups as $category => $spaces): ?>
      <div
        class="tab-pane <?php if ($category == $active): print 'active'; endif; ?>"
        id="<?php print 'tab-' . $category; ?>">
